...
void correz(){
  ...
  char ofile[80];
  strcpy(ofile,dest2);
   char infile1[80] = "PERCORREZIONI_EEE_PISA01TestRun4Telescopes_20140507_014455.txt";
 ...
 //  }
  ...
  for (int i = 0; i < 24; i++) {corry1[i] = up1[i]-low1[i]; corry2[i] = up2[i]-low2[i]; corry3[i] = up3[i] - low3[i];}
   ...
   for (int i = 0; i < 24; i++) {
     if(corry1[i] != 0) corry1[i] = 158.0/corry1[i]; else corry1[i] = 0; //y rescaling
       if(corry2[i] != 0) corry2[i] = 158.0/corry2[i]; else corry2[i] = 0; //y rescaling
     if(corry3[i] != 0) corry3[i] = 158.0/corry3[i]; else corry3[i] = 0; //y rescaling
   }

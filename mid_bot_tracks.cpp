...
void mid_bot_tracks(){
  ...
  // double parameter = 0;
   ifstream run("../Data/CORR_EEE_Prova_midbottom9000__20140530_184644.txt"); //INPUT FILE  
  TFile rfile("Distmidbot_9000.root","RECREATE");
 ...
 //Controllo la bontà dell'evento: ha almeno un hit per camera (top_mid)?
  if (ch2 == 0 || ch3 == 0){/*cout << "EVENTO NON BUONO" << endl;*/ 
    j = 0; 
    delete [] hit;
    // cout << point::n << endl;
    delete[] x;
   ...
   //Riempiamo gli istogrammi di theta e phi se il fit è andato bene. Se è verticale considero solamente una sezione
      ...
      if((yxtempchi < 6.63) && (yztempchi < 6.63) && (zxtempchi < 6.63) ){
  ...
  //   disphi->Draw();
   ...
   disz->Write();
    ...
    dischi->Write();
     ...
     cout << "EFFICIENZA CON TRACKING: " << double(efft3)/double(eff2+eff3) << endl;
     cout << "EFFICIENZA CON TRACKING CON FIT BUONI: " << double(efft3)/double(efft3+eff2);
}
